"""Feature registry. Host dispatches `[klipper_extras <kind>]` here only.
...
from .pause_resume import (
    GCODES as PAUSE_RESUME_GCODES,
    KIND as PAUSE_RESUME_KIND,
    load_feature as load_pause_resume,
)
...
FEATURE_LOADERS = {
    ...
    HOOK_KIND: load_hook,
    PAUSE_RESUME_KIND: load_pause_resume,
...
# Values are str or tuple[str, ...].
FEATURE_GCODES = {
    BED_KIND: BED_GCODE,
    FORM_TIP_KIND: FORM_TIP_GCODE,
    PAUSE_RESUME_KIND: PAUSE_RESUME_GCODES,
...
def feature_gcode_names(kind: str) -> list:
    raw = FEATURE_GCODES.get(kind)
    if raw is None:
        return []
