# Klipper Extras — shared Klipper extra (foundation for other extras)
#
# Copyright (C) 2026 Klipper Extras contributors
# SPDX-License-Identifier: GPL-3.0-or-later
#
# Install: copy/symlink this package to klippy/extras/klipper_extras/
# Config section: [klipper_extras]
...
class KlipperExtras:
    def __init__(self, config):
        ...
        self.gcode.register_command(
            "EXTRAS_STATUS",
            self.cmd_EXTRAS_STATUS,
            desc=msg.help_extras_status(),
        )
        self.gcode.register_command(
            "EXTRAS_VERSION",
            self.cmd_EXTRAS_VERSION,
            desc=msg.help_extras_version(),
        )
    ...
    def _ready_announce_parts(self):
        ...
        if s is None:
            return None, []
        return msg.ready_banner(KLIPPER_EXTRAS_VERSION), msg.ready_detail_lines(
    ...
    def _announce_ready_console_timer(self, eventtime):
        ...
        for line in lines:
            try:
                ...
                logging.debug("%s", msg.line("ready console emit failed"), exc_info=True)
    ...
    def cmd_EXTRAS_STATUS(self, gcmd):
        level = self._configured_log_level()
    ...
    def cmd_EXTRAS_VERSION(self, gcmd):
        gcmd.respond_info(msg.version_report(KLIPPER_EXTRAS_VERSION))

    def get_status(self, eventtime):
        status = {
            "version": KLIPPER_EXTRAS_VERSION,
